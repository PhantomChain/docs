# How to write an Phantom Core Plugin

In this guide, you will find the information which can enable you to write a proper Phantom Core plugin, for use in your own Phantom deployments; both in the case of Phantom Core nodes and your own BridgeChain nodes.

## Setup
Many components are required to have a proper environment setup for the development of your Phantom Core plugin.
...
### Plugin Skeleton
Make sure you are in the Phantom Core folder cloned from the official [GitHub repo](https://github.com/PhantomChain/core).
...
```sh
lerna add --scope=@phantomchain/demo-plugin big-number --dev
```
...
### Testing
Testing is streamlined by the `test-utils` Phantom Core package.
...
Lastly, before running our tests, we need to include `@phantomchain/demo-plugin` in the testnet configuration for it to load when running the suite.
...
## Conclusion
In the end, you should be able to write your own plugin for Phantom Core, with full interoperability with the existing core packages and other dependencies that might be required for your project.
