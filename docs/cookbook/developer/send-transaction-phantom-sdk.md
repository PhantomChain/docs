# How to Send Your First Transaction with PHANTOM SDK

In this recipe, we'll use PHANTOM's Client and Crypto libraries to send an PHANTOM transaction programmatically. You'll see how the Client and Crypto libraries work together to provide a comprehensive base for any PHANTOM project, and you'll learn how to configure PHANTOM's SDKs to work alongside your testnet.

## Getting Started

For this recipe, we'll be using a testnet to send our first transaction. A **testnet** is an PHANTOM blockchain that runs exclusively on your own computer, and for that reason it's a great place to try out new concepts before porting them to devnet or mainnet. If you haven't set up your own testnet yet, check out our recipe on [How to Setup Dev Environment](/cookbook/developer/setup-dev-environment.html) and follow the instructions there before continuing.
...
Sending complex requests directly in your browser's URL window by typing them out is neither fun nor efficient. Instead, we can leverage one of PHANTOM's most popular features: the first-party SDKs available in virtually all major programming languages. These SDKs streamline interacting with the PHANTOM blockchain no matter your language; we'll be using the JavaScript SDK for the sake of continuity, but you're welcome to copy our approach with the SDK of your choice.

All SDK implementations are split into two libraries. The `client` library is responsible for sending requests and interpreting responses between your application and PHANTOM nodes. By contrast, the `crypto` library is responsible for many of the most common PHANTOM-related calculations your app will make: building transactions to send, signing transactions with passphrases, changing configuration settings, and so on.
...
```sh
yarn add @phantomchain/client @phantomchain/crypto
```
...
2. The version of the PHANTOM API we want to access. 1 is for PHANTOM v1, 2 is for PHANTOM v2. If possible, always go full v2.
...
At some point during the tutorial, you might run into some network-related bugs. By default, as of the time of writing, the `crypto` and `client` libraries are configured to work on devnet unless specifically told otherwise. As devnet is the ecosystem-wide testing ground for the new PHANTOM Core, setting devnet as the default ensures that community testers can set up nodes and applications with a minimum of overhead.
...
To do so, import the `configManager` package from `crypto`, then tell it to use the `PHANTOM` network on `testnet` settings:

```js
const Client = require('@phantomchain/client')
const { configManager } = require('@phantomchain/crypto')

configManager.setFromPreset('PHANTOM', 'testnet')
...
Now, it's probably important to note here that, although we'll be picking passphrases to use for both the sending and receiving accounts, typically you don't need the recipient's password to send them a transaction. However, as we're the only users on our own testnet, we can't exactly ask a friend for their PHANTOM address so we can send them test transactions.
...
In order to go from our recipient passphrase to our recipient testnet address, we'll need to pull off a crypto Inception and require the `crypto` module from the `crypto` package. This library contains most of the cryptographic functions we'll need to work with PHANTOM, from signing transactions to, as we'll soon see, deriving hashes from other hashes.
...
```js
const Client = require('@phantomchain/client')
const { crypto, configManager } = require('@phantomchain/crypto')
...
const transaction = transactionBuilder
  ...
  .amount(20 * 1e8) // 20 PHANTOM, multiplied by 10^8 to get PHANTOMtoshi value
...
With everything in place, it's time to send some (T)PHANTOM. Using the `client` package from the beginning of the recipe, let's run through an example of how to send a transaction:
```js
testnetClient
...
```js
const Client = require('@phantomchain/client')
const { crypto, transactionBuilder, configManager } = require('@phantomchain/crypto')
